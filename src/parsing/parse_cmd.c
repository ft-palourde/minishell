...
void		merge_word_tokens(t_token *token, char **env);
static void	set_cmd_args(t_token *token, char **env);
...
void	merge_word_tokens(t_token *token, char **env)
{
	...
	{
		token->data->cmd->args[i] = str_expand(ft_strdup(cursor->str), env);
